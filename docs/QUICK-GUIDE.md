...
You only need `kubectl` and a Kubernetes cluster. No cluster? Spin up a local one with
...
The repo ships a tiny, ready-to-run example — `networking-demo` (three satellites and
one ground station). Apply the shared hardware profiles once, then the example:
...
```bash
kubectl -n networking-demo get experiment -w   # STATE goes Init -> Ready -> Ongoing
kubectl -n networking-demo get fsnodes -w      # per-node phase, battery, position
```
...
```bash
kubectl -n networking-demo port-forward svc/yass-web-ui 8080:80
# -> open http://localhost:8080
```

Want dashboards too? Port-forward Grafana (login `admin` / `yass-admin`):
